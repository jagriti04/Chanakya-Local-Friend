...
from chanakya.services.mcp_artifact_tools_server import _create_artifact, _update_artifact
...
def test_create_artifact_writes_file_and_registers_record(monkeypatch, tmp_path: Path) -> None:
    ...
    workspace = sandbox_workspace.resolve_shared_workspace("req_artifact_tool", create=False)
...
def test_update_artifact_rewrites_content_and_tracks_latest_request(monkeypatch, tmp_path: Path) -> None:
    ...
    workspace = sandbox_workspace.resolve_shared_workspace("req_first", create=False)
    artifact_file = workspace / artifact["path"]
