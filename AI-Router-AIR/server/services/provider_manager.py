...
class ProviderManager:
    ...
    async def refresh_models(self) -> List[Dict[str, Any]]:
        ...
        # Debug logging
        logging.info(f"Refreshing models. Found {len(self.providers)} providers.")
        for p in self.providers:
            logging.info(f"Provider: {p.name} ({p.type}) -> {p.base_url}")
        ...
        for res in results:
            if isinstance(res, list):
                ...
                logging.warning(f"Error during fetch: {res}")
