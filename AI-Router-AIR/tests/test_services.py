...
@pytest.mark.asyncio
async def test_provider_manager_refresh_models_keeps_previous_provider_models_on_failure():
    pm = ProviderManager()
    pm.models_cache["by_provider"] = {
        "P1": [{"id": "groq-model", "provider_name": "P1", "provider_type": "llm"}],
        "P2": [{"id": "stt-model", "provider_name": "P2", "provider_type": "stt"}],
    }
    pm.models_cache["all"] = [
        {"id": "groq-model", "provider_name": "P1", "provider_type": "llm"},
        {"id": "stt-model", "provider_name": "P2", "provider_type": "stt"},
    ]

    mock_providers = [
        ProviderConfig(name="P1", base_url="https://api.groq.com/openai/v1", api_key="k1", type="llm"),
        ProviderConfig(name="P2", base_url="http://p2/v1", api_key="k2", type="stt"),
    ]
    ...
    with patch("server.services.provider_manager.settings") as mock_settings:
        ...
        pm._fetch_models_from_provider.side_effect = [
            ...
            [{"id": "stt-model-fresh", "provider_name": "P2", "provider_type": "stt"}],
        ]

        models = await pm.refresh_models()

        assert {m["id"] for m in models} == {"groq-model", "stt-model-fresh"}
        assert pm.models_cache["by_provider"]["P1"][0]["id"] == "groq-model"
        assert pm.models_cache["by_provider"]["P2"][0]["id"] == "stt-model-fresh"
